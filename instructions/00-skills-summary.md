...
- For choice list (`choiceList`, inline or a named toolbox list from `lists.json`, auto-instantiated/cleared with the form): `saveAs: "value"|"reference"` picks whether the bidirectional data source holds the literal text or a 1-based numeric reference -- initialize to `0` (reference) or a placeholder message (value)
- Hierarchical drop-down (`dataSourceTypeHint: "integer"` alone) is backed by a hierarchical list reference (integer) from `New list`/`Load list`, assigned directly to the data source; must `Clear list` on unload since it isn't cleared automatically
